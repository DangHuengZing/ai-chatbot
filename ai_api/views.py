    ...
    conversation_list = [
        {
            ...
            'title': ChatMessage.objects.filter(user=request.user, conversation_id=conv['conversation_id'])
                .order_by('timestamp').first().content[:30] or '未命名对话'
        }
        for conv in conversations if ChatMessage.objects.filter(user=request.user, conversation_id=conv['conversation_id']).exists()
    ]
...
def stream_chat(request):
    ...
    try:
        ...
        history = ChatMessage.objects.filter(user=request.user, conversation_id=conversation_id).order_by('timestamp').values('role', 'content')[:10]
        messages = [{'role': 'assistant' if msg['role'] == 'ai' else msg['role'], 'content': msg['content']} for msg in history]
